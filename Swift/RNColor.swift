...
class RNDefaultColor: NSObject,RNColorProtocol {
    static let sharedInstance = RNDefaultColor()
...
/// Dark Colors
class RNDarkColor: NSObject,RNColorProtocol {
    static let sharedInstance = RNDarkColor()
    fileprivate override init(){
        super.init()
    }
...
class RNColor :NSObject  {
    fileprivate static let STYLE_KEY = "styleKey"
    
    static let RNColorStyleDefault = "Default"
    static let RNColorStyleDark = "Dark"
    
    fileprivate static var _colors:RNColorProtocol?
    static var colors :RNColorProtocol {
        get{
            
            if let c = RNColor._colors {
                return c
            }
            else{
                if RNColor.sharedInstance.style == RNColor.RNColorStyleDefault{
                    return RNDefaultColor.sharedInstance
                }
                else{
                    return RNDarkColor.sharedInstance
                }
            }
            
        }
        set{
            RNColor._colors = newValue
        }
    }
    
    dynamic var style:String
    static let sharedInstance = RNColor()
    fileprivate override init(){
        if let style = RNSettings.sharedInstance[RNColor.STYLE_KEY] {
            self.style = style
        }
        else{
            self.style = RNColor.RNColorStyleDefault
        }
        super.init()
    }
    func setStyleAndSave(_ style:String){
        ...
        if style == RNColor.RNColorStyleDefault {
            RNColor.colors = RNDefaultColor.sharedInstance
        }
        else{
            RNColor.colors = RNDarkColor.sharedInstance
        }
        
        self.style = style
        RNSettings.sharedInstance[RNColor.STYLE_KEY] = style
    }
...
extension NSObject {
    ...
    var thmemChangedHandler:ThemeChangedClosure? {
        ...
        set{
            ...
            //设置KVO监听
            self.kvoController.observe(RNColor.sharedInstance, keyPath: "style", options: [.initial,.new] , block: {[weak self] (nav, color, change) -> Void in
                self?.thmemChangedHandler?(RNColor.sharedInstance.style)
                }
            )
        }
    }
